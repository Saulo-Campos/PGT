...
import Aplicativo.Controle.Fonoaudiologo.MetodoControle;
import Aplicativo.Modelo.Saude.Metodo;
import Aplicativo.Modelo.Sistema.Dark;

public class MetodoTela extends JFrame {
	...
	Metodo metodoSelecionado = new Metodo();
	MetodoControle metodoControle = new MetodoControle();
	int indexMetodo = -1;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MetodoTela frame = new MetodoTela();
	 ...
	 */
	public MetodoTela() {
		setTitle("Cadastro de Metodo");
		setIconImage(Toolkit.getDefaultToolkit()
		    .getImage(MetodoTela.class.getResource("/Multmidia/Imagens/logo/logoGenerico/avart 2v10.png")));
		...
		btnNovo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				metodoControle.AtualizarTabelaMetodo(tabela);
				limpacampos();
			}
		});
		...
		btnSalvar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// Metodo.setSenha(txtSenha.getText());
				// Metodo.setNomeCompleto(txtNomeCompleto.getText());

				if (txtId.getText().isEmpty()) {
					if (metodoControle.adicionar(metodo)) {
						metodoControle.AtualizarTabelaMetodo(tabela);
						limpacampos();
						txtItemSelecionado.setText("nenhum");
					}
				} else if (CheckItemSelecionado(indexMetodo)) {
					metodoSelecionado.setId((long) tabela.getValueAt(indexMetodo, 0));
					metodoSelecionado.setNome(txtNome.getText());
					// MetodoSelecionado.setSenha(txtSenha.getText());
					// MetodoSelecionado.setNomeCompleto(txtNomeCompleto.getText());

					if (metodoControle.atualizar(metodoSelecionado)) {
						metodoControle.AtualizarTabelaMetodo(tabela);
						limpacampos();
						txtItemSelecionado.setText("nenhum");
		...
		tabela.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2 && !e.isConsumed()) {
					...
					indexMetodo = tabela.getSelectedRow();
					txtItemSelecionado.setText("" + (indexMetodo + 1));

					Long idMetodo = (Long) tabela.getValueAt(indexMetodo, 0);
					metodoSelecionado = metodoControle.pegarPorId(idMetodo);

					txtId.setText(metodoSelecionado.getId().toString());
					txtNome.setText(metodoSelecionado.getNome());
					// txtNomeCompleto.setText(MetodoSelecionado.getNomeCompleto());
					// txtSenha.setText(MetodoSelecionado.getSenha());
				}
			}
		});
