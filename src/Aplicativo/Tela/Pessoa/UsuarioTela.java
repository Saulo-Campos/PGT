...
public class UsuarioTela extends JFrame {
	...
	public UsuarioTela() {
		...
		setIconImage(Toolkit.getDefaultToolkit()
		    .getImage(UsuarioTela.class.getResource("/Multmidia/Imagens/logo/logoGenerico/avart 2v10.png")));
		...
		btnNovo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				usuarioControle.AtualizarTabelaUsuario(tabela);
				limpacampos();
			}
		});
		...
		btnSalvar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (txtId.getText().isEmpty()) {
				...
				} else if (CheckItemSelecionado(indexUsuario)) {
					usuarioSelecionado.setId((long) tabela.getValueAt(indexUsuario, 0));
					...
					if (usuarioControle.atualizar(usuarioSelecionado)) {
						usuarioControle.AtualizarTabelaUsuario(tabela);
						limpacampos();
						txtItemSelecionado.setText("nenhum");
		...
		btnRemover.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (CheckItemSelecionado(indexUsuario)) {
					usuarioControle.remover(Long.parseLong(txtId.getText()));
					usuarioControle.AtualizarTabelaUsuario(tabela);
					limpacampos();
				}
			}
		...
		tabela.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2 && !e.isConsumed()) {
					...
					indexUsuario = tabela.getSelectedRow();
					txtItemSelecionado.setText("" + (indexUsuario + 1));

					Long idUsuario = (Long) tabela.getValueAt(indexUsuario, 0);
					usuarioSelecionado = usuarioControle.pegarPorId(idUsuario);

					txtId.setText(usuarioSelecionado.getId().toString());
