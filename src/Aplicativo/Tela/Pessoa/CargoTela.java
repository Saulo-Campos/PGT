...
import Aplicativo.Controle.Pessoa.CargoControle;
import Aplicativo.Modelo.Pessoa.Cargo;
import Aplicativo.Modelo.Sistema.Dark;


public class CargoTela extends JFrame {
	...
	public CargoTela() {
		...
		setIconImage(Toolkit.getDefaultToolkit()
				.getImage(CargoTela.class.getResource("/Multmidia/Imagens/logo/logoGenerico/avart 2v10.png")));
		...
		btnNovo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				CargoControle.AtualizarTabelaCargo(tabela);
				limpacampos();
			}
		});
		...
		btnSalvar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//Cargo.setSenha(txtSenha.getText());
				//Cargo.setNomeCompleto(txtNomeCompleto.getText());

				if (txtId.getText().isEmpty()) {
					if (CargoControle.adicionar(Cargo)) {
						CargoControle.AtualizarTabelaCargo(tabela);
						limpacampos();
						txtItemSelecionado.setText("nenhum");
					}
				} else if (CheckItemSelecionado(indexCargo)) {
					CargoSelecionado.setId((long) tabela.getValueAt(indexCargo, 0));
					CargoSelecionado.setNome(txtNome.getText());
					//CargoSelecionado.setSenha(txtSenha.getText());
					//CargoSelecionado.setNomeCompleto(txtNomeCompleto.getText());

					if (CargoControle.atualizar(CargoSelecionado)) {
						CargoControle.AtualizarTabelaCargo(tabela);
						limpacampos();
						txtItemSelecionado.setText("nenhum");
		...
		tabela.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2 && !e.isConsumed()) {
					...
					indexCargo = tabela.getSelectedRow();
					txtItemSelecionado.setText("" + (indexCargo + 1));

					Long idCargo = (Long) tabela.getValueAt(indexCargo, 0);
					CargoSelecionado = CargoControle.pegarPorId(idCargo);

					txtId.setText(CargoSelecionado.getId().toString());
					txtNome.setText(CargoSelecionado.getNome());
					//txtNomeCompleto.setText(CargoSelecionado.getNomeCompleto());
					//txtSenha.setText(CargoSelecionado.getSenha());
				}
			}
		});
