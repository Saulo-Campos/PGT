...
import Aplicativo.Controle.Pessoa.ClienteControle;
...
public class ClienteTela extends JFrame {
	...
	public ClienteTela() {
		...
		setIconImage(Toolkit.getDefaultToolkit()
		    .getImage(ClienteTela.class.getResource("/Multmidia/Imagens/logo/logoGenerico/avart 2v10.png")));
		...
		btnNovo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ClienteControle.AtualizarTabelaCliente(tabela);
				limpacampos();
			}
		});
		...
		btnSalvar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (txtId.getText().isEmpty()) {
					if (ClienteControle.adicionar(Cliente)) {
						ClienteControle.AtualizarTabelaCliente(tabela);
						limpacampos();
						txtItemSelecionado.setText("nenhum");
					}
				} else if (CheckItemSelecionado(indexCliente)) {
					ClienteSelecionado.setId((long) tabela.getValueAt(indexCliente, 0));
					ClienteSelecionado.setNome(txtNome.getText());
					ClienteSelecionado.setSenha(txtSenha.getText());
					ClienteSelecionado.setNomeCompleto(txtNomeCompleto.getText());

					if (ClienteControle.atualizar(ClienteSelecionado)) {
						ClienteControle.AtualizarTabelaCliente(tabela);
						limpacampos();
						txtItemSelecionado.setText("nenhum");
		...
		tabela.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2 && !e.isConsumed()) {
					...
					indexCliente = tabela.getSelectedRow();
					txtItemSelecionado.setText("" + (indexCliente + 1));

					Long idCliente = (Long) tabela.getValueAt(indexCliente, 0);
					ClienteSelecionado = ClienteControle.pegarPorId(idCliente);

					txtId.setText(ClienteSelecionado.getId().toString());
					txtNome.setText(ClienteSelecionado.getNome());
					txtNomeCompleto.setText(ClienteSelecionado.getNomeCompleto());
					txtSenha.setText(ClienteSelecionado.getSenha());
				}
			}
		});
